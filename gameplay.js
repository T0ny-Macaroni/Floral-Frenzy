...
window.onload = function () {
    ...
    function drawFrame() {
        ...
        context.fillStyle = "#ffffff";
        context.font = "24px Verdana";
        context.fillText("Match3 Delakdev", 10, 30);
    ...
    function renderTiles() {
        for (var i = 0; i < level.columns; i++) {
            for (var j = 0; j < level.rows; j++) {
                ...
                if (level.tiles[i][j].type >= 0) {
                    ...
                    drawTile(coord.tilex, coord.tiley, col[0], col[1]);
                }
                ...
                if (level.selectedtile.selected) {
                    if (level.selectedtile.column == i && level.selectedtile.row == j) {
                        // Draw a red tile
                        drawTile(coord.tilex, coord.tiley, 'rgba(255,255,255,0.5)', col[1]);
                    }
                }
            }
        }
        ...
        if (gamestate == gamestates.resolve && (animationstate == 2 || animationstate == 3)) {
            ...
            drawTile(coord1.tilex, coord1.tiley, '#000000', col1[1]);
            drawTile(coord2.tilex, coord2.tiley, '#000000', col2[1]);

            // Change the order, depending on the animation state
            if (animationstate == 2) {
                // Draw the tiles
                drawTile(coord1shift.tilex, coord1shift.tiley, col1[0], col1[1]);
                drawTile(coord2shift.tilex, coord2shift.tiley, col2[0], col2[1]);
            } else {
                // Draw the tiles
                drawTile(coord2shift.tilex, coord2shift.tiley, col2[0], col2[1]);
                drawTile(coord1shift.tilex, coord1shift.tiley, col1[0], col1[1]);
    ...
    function drawTile(x, y, hex, imageID) {
        image = document.getElementById(imageID);
        context.beginPath();
        context.arc(x + 32, y + 32, 30, 0, 2 * Math.PI, false);
        context.fillStyle = hex;
        context.fill();
        context.closePath();
        context.drawImage(image, x + 20, y + 20, 24, 24);

    }

    // Render clusters (à remplacer par animation éclatement)
                ...
                if (checkcluster) {
                    if (matchlength >= 3) {
                        ...
                        clusters.push({
                            ...
                            length: matchlength, horizontal: true, tilecolor: getTileColor(level.tiles[i + 1 - matchlength][j].type)
                        });
                    }
                ...
                if (checkcluster) {
                    if (matchlength >= 3) {
                        ...
                        clusters.push({
                            ...
                            length: matchlength, horizontal: false, tilecolor: getTileColor(level.tiles[i][j + 1 - matchlength].type)
                        });
                    }
    ...
    // Play a sound based on match numbers
    function playSound(number) {

        let sound = new Audio('3matches.wav');
        if (number > 4) {
            sound = new Audio('5matches.wav');
        }
        if (number > 3) {
            sound = new Audio('4matches.wav');
        }
    ...
    //return the color of a giver tile
    function getTileColor(type) {
        return tilecolors[type][1];
    }
